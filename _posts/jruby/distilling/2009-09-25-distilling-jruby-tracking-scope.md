...
{% assign caption='Figure 1: Note the package "org.jruby.parser"' %}
{% assign align='right' %}
{% assign src='/distilling/jruby/static_scope1.png' %}
{% include image.html %}
...
Note that in the local case, it attempts to look up the variable by name, receiving something called a 'slot'. If it can't find a variable in the declaration stack, it ultimately assumes that the usage of the variable in this case must be a 'call node' - which in turn translates to a method call (which I have already [discussed in a previous article](/2009/09/16/distilling-jruby-method-dispatching-101/)). Ruby will then try to find that method at runtime (remember, methods, unlike local variables, can be added and removed at runtime freely).
...
{% assign caption='Figure 2: Arity-Based Scopes' %}
{% assign align='left' %}
{% assign src='distilling/jruby/dynamic_scope.png' %}
{% include image.html %}
...
s you can see, it simply returns the value from the current dynamic scope (as determined by the ThreadContext, which we'll discuss shortly). You'll see it uses 'getIndex()' and 'getDepth()' methods - here those are, using the mathematical bit-masking as discussed previously:
...
{% assign caption='Figure 3: Call Stack Example' %}
{% assign src='/distilling/jruby/call_stack.png' %}
{% include image.html %}
